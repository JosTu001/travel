  ...
  <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen}">
    ...
    <div class="flex flex-col flex-1 w-full">
      ...
      <main class="h-full pb-16 overflow-y-auto">
        <!-- THÊM TUYẾN -->
        <form action="themchuyen.php" method="post">
          <div class="container grid px-6 mx-auto">
            ...
            <div class="px-6 my-6">
              ...
              <!-- <input type="hidden" name="TC_Ma" value="<?php echo $row['C_Ma']; ?>" /> -->
            </div>
        </form>
            ...
            <table class="w-full whitespace-no-wrap">
              <thead>
                <tr class=" text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                  <th class="px-4 py-3" style="width: 12%">Mã Chuyến</th>
                  <th class="px-4 py-3" style="width: 12%">Thời gian tạo</th>
                  <th class="px-4 py-3" style="width: 12%">Nhân viên phụ trách</th>
                  <th class="px-4 py-3" style="width: 12%">Tên Tuyến</th>
                  <th class="px-4 py-3" style="width: 15%">Thời Gian Khởi Hành</th>
                  <th class="px-4 py-3" style="width: 15%">Thời Gian Đến</th>
                  <th class="px-4 py-3" style="width: 20%">Bến xe khởi hành</th>
                  <th class="px-4 py-3" style="width: 20%">Bến xe đến</th>
                  <th class="px-4 py-3" style="width: 15%">Đơn Giá</th>
                  <th class="px-4 py-3" style="width: 20%">Xe</th>
                  <th class="px-4 py-3" style="width: 20%">Tài Xế</th>
        ...
        ?>
              ...
              // $sl = "SELECT * FROM chuyen a, nhanvien b, taixe c, xe d, tuyenchay f , benxe g where a.BXKH_Ma=g.BX_Ma  and a.NV_Ma=b.NV_Ma and a.TX_Ma=c.TX_Ma and a.X_Ma=d.X_Ma and a.TC_Ma=f.TC_Ma and (f.TC_Ten LIKE '%$noidung%' or C_DonGia LIKE '%$noidung%' or a.C_ThoiGianXP LIKE '%$noidung%' or c.TX_Ten LIKE '%$noidung%')";
              // $qu = mysqli_query($conn, $sl);

              $item_per_page = !empty($_GET['per_page'])?$_GET['per_page']:10;
                        $current_page = !empty($_GET['page'])?$_GET['page']:1; //Trang hiện tại
                        $offset = ($current_page - 1) * $item_per_page;
                        $products = mysqli_query($conn, "SELECT * FROM chuyen a, nhanvien b, taixe c, xe d, tuyenchay f ,benxe g where a.BXKH_Ma = g.BX_Ma and a.NV_Ma=b.NV_Ma and a.TX_Ma=c.TX_Ma and a.X_Ma=d.X_Ma and a.TC_Ma=f.TC_Ma and (f.TC_Ten LIKE '%$noidung%' or C_DonGia LIKE '%$noidung%' or a.C_ThoiGianXP LIKE '%$noidung%' or c.TX_Ten LIKE '%$noidung%' or NV_Ten LIKE '%$noidung%') ORDER BY `C_Ma` DESC  LIMIT " . $item_per_page . " OFFSET " . $offset);
                        $totalRecords = mysqli_query($conn, "SELECT * FROM chuyen ");
                  ...
                  <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3">
                      <div class="flex items-center text-sm">
                        <div>
                          <p class="font-semibold"><?php echo $row['C_Ma']; ?></p>
                      ...
                      <div class="flex items-center text-sm">
                        <div>
                          ...
                          <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                              <?php 
                          // echo $row["KH_NgaySinh"];\
                          if($row["C_ThoiGianTao"] == '0000-00-00 00:00:00'){
                            echo '';
                          }else{

                            $date = date_create($row["C_ThoiGianTao"]);
                    ...
                    <td class="px-4 py-3 text-xs">
                      <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                        <?php $date = date_create($row["C_ThoiGianXP"]);
                        echo date_format($date, "d-m-Y H:i:s"); 
                    ...
                    <td class="px-4 py-3 text-xs">
                      <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                        <?php $date = date_create($row["C_ThoiGianDen"]);
                        echo date_format($date, "d-m-Y H:i:s"); ?>
                      </span>
                    </td>
                    ...
                    <td class="px-4 py-3 text-sm">
                      ...
                      $q = "SELECT * FROM benxe a, chuyen b where a.BX_Ma=b.BXKH_Ma and a.BX_Ma='" . $row['BXKH_Ma'] . "'";
                      ...
                      $sq = "SELECT * FROM benxe a, chuyen b where a.BX_Ma=b.BXD_Ma and a.BX_Ma='" . $row['BXD_Ma'] . "'";
                      $qe = mysqli_query($conn, $sq);
                      $ro = mysqli_fetch_assoc($qe)
                      ?>
                    <td class="px-4 py-3 text-sm">
                      <?php echo $ro['BX_Ten']; ?>
                   ...
                   <!-- GIÁ -->
                    <td class="px-4 py-3 text-sm">
                      <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                        <?php echo number_format($row["C_DonGia"]); ?>
                      </span>
                    </td>
                            ...
                            <div
                              ...
                              >
                                <img
                                  ...
                                  src="../img/Xe/ghe/<?php //echo $row["C_Hinh"]; ?>"
                                  loading="lazy"
                                />
                                <div
                    ...
                    <!-- <td class="px-4 py-3 text-sm">
                      <div class="">
                        <img width="50%" class="" src="../img/Xe/ghe/<?php //echo $row["C_Hinh"]; ?>" loading="lazy" />
                    ...
                    <td class="px-4 py-3">
                      ...
                      <form action="" method="post">
                          <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Sua" href="chitietLich.php?this_id=<?php echo $row['C_Ma'] ?> ">
                          <i class="fa fa-eye"></i>

                          </a>
                          <input type="hidden" name="C_Ma" value="<?php echo $row['C_Ma']; ?>" />
                        </form>


                        <!-- SỬA -->
                        <form action="" method="post">
                          <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Sua" href="capnhatchuyen.php?this_id=<?php echo $row['C_Ma'] ?> ">
                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <form action="" method="post">
                          <a style="color: red;" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Xoa" class="btn btn-info btn-sm" href="xoachuyen.php?this_id=<?php echo $row['C_Ma'] ?> ">
