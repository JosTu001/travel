    ...
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen}">
        ...
        <div class="flex flex-col flex-1 w-full">
            ...
            <main class="h-full pb-16 overflow-y-auto">
                <!-- THÊM TUYẾN -->
                <form action="themghe.php" method="post">
                        ...
                        <table class="w-full whitespace-no-wrap">
                            <thead>
                                <tr class="text-center text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                                    ...
                                    <th class="px-4 py-3 " style="width: 20%">STT</th>
                                    <th class="px-4 py-3 " style="width: 20%">Xe</th>
                                    <th class="px-4 py-3" style="width: 10%">Số lượng ghế</th>
                                    <th class="px-4 py-3" style="width: 10%">Số ghế đã đặt</th>
                                    <th class="px-4 py-3" style="width: 20%">Số ghế trống</th>
                                    <th class="px-4 py-3" style="width: 20%">Ghế</th>
                                   
                                    <th class="px-4 py-3"style="width: 10%">Hành độnng</th>

                                </tr>
                            </thead>
                            <?php

                            // $sql = "SELECT * FROM ghe a, xe b where a.X_Ma=b.X_Ma group by X_BienSo ";
                            // $query = mysqli_query($conn, $sql);

                            $item_per_page = !empty($_GET['per_page'])?$_GET['per_page']:10;
                            $current_page = !empty($_GET['page'])?$_GET['page']:1; //Trang hiện tại
                            $offset = ($current_page - 1) * $item_per_page;
                            $products = mysqli_query($conn, "SELECT * FROM ghe a, xe b where a.X_Ma=b.X_Ma group by X_BienSo ORDER BY `X_BienSo` ASC  LIMIT " . $item_per_page . " OFFSET " . $offset);
                                ...
                                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                    <tr class="text-gray-700 dark:text-gray-400">
                                        ...
                                        <td class="px-4 py-3 text-sm text-center">
                                            <?php $sl = "select count(*) as 'soghe' from xe,ghe where xe.X_Ma=ghe.X_Ma   and xe.X_Ma='".$row['X_Ma']."' ";
                                        ...
                                        <td class="px-4 py-3 text-sm text-center">
                                            <?php $sl = "select count(*) as 'dadat' from xe,ghe where xe.X_Ma=ghe.X_Ma   and xe.X_Ma='".$row['X_Ma']."' and ghe.G_TrangThai= 1";
                                            $res = mysqli_query($conn, $sl);
                                            while ($ro = mysqli_fetch_array($res)) { ?>
                                                <?php echo $ro["dadat"]; ?> &nbsp;
                                        ...
                                        <td class="px-4 py-3 text-sm text-center">
                                            <?php $sl = "select count(*) as 'ghetrong' from xe,ghe where xe.X_Ma=ghe.X_Ma   and xe.X_Ma='".$row['X_Ma']."' and ghe.G_TrangThai= 0";
                                            $res = mysqli_query($conn, $sl);
                                            while ($ro = mysqli_fetch_array($res)) { ?>
                                                <?php echo $ro["ghetrong"]; ?> &nbsp;

                                            <?php } ?>
                                        </td>

                                        <td class="px-4 py-3 text-sm ">
                                            <?php $sl = "select * from xe,ghe where xe.X_Ma=ghe.X_Ma  and xe.X_Ma='".$row['X_Ma']."'";
                                            $res = mysqli_query($conn, $sl);
                                            while ($ro = mysqli_fetch_array($res)) { 
                                                if($ro["G_TrangThai"]==1){
                                                    ?> <span style="color:red"><?php echo $ro["G_MaSoGhe"]; ?> &nbsp;</span> <?php
                                        ...
                                        <td class="px-4 py-3">
                                            ...
                                            <div class="flex items-center space-x-4 text-sm">
                                                ...
                                                <form action="" method="post">
                                                    <a style="margin-left:2rem" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Sua" href="xemctghe.php?this_id=<?php echo $row['X_Ma'] ?> ">
                                                        <i class="fa fa-eye"></i>
