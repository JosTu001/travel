    ...
    <div class="flex flex-col flex-1 w-full">
      ...
      <main class="h-full pb-16 overflow-y-auto">
        ...
        <?php //} ?>
              ...
              // $sql = "SELECT * FROM khachhang WHERE ( KH_Ten LIKE '%$noidung%' or KH_Email LIKE '%$noidung%' or KH_Sdt LIKE '%$noidung%') ";
              // $query = mysqli_query($conn, $sql);

              $item_per_page = !empty($_GET['per_page'])?$_GET['per_page']:10;
              $current_page = !empty($_GET['page'])?$_GET['page']:1; //Trang hiện tại
              $offset = ($current_page - 1) * $item_per_page;
              $products = mysqli_query($conn, "SELECT * FROM khachhang WHERE ( KH_Ten LIKE '%$noidung%' or KH_Email LIKE '%$noidung%' or KH_Sdt LIKE '%$noidung%') ORDER BY `KH_Ma` ASC  LIMIT " . $item_per_page . " OFFSET " . $offset);
                  ...
                  <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3">
                      <div class="flex items-center text-sm">
                        <div>
                          <p class="font-semibold"><?php echo $row['KH_Ma']; ?></p>
                    ...
                    <!-- TÊN TUYẾN -->
                    <?php
                          if ($row['KH_TrangThai'] == 1) {
                          $row['KH_TrangThai'] = "Online";
                          ?>
                             <td class="px-4 py-3 text-sm">
                            <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                              <?php  echo $row['KH_TrangThai']; ?>
                            </span>
                            </td>

                          <?php
                          } else  {
                          $row['KH_TrangThai'] = "Offline";
                          ?>
                             <td class="px-4 py-3 text-sm">
                    ...
                    <td class="px-4 py-3 text-sm">
                    <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                      <?php   echo date_format(date_create($row['KH_NgayDK']), "d-m-Y H:i:s ");  ?>
                    </span>
                    </td>
                    ...
                    <td class="px-4 py-3 text-sm">
                      ...
                      if($row["KH_NgaySinh"] == '0000-00-00'){
                        echo '';
                      }else{

                        $date = date_create($row["KH_NgaySinh"]);
                    ...
                    <!-- ĐỊA CHỈ -->
                    <td class="px-4 py-3 text-sm">
                      <?php echo $row['KH_DiaChi']; ?>
                    </td>

                    <!-- HINH -->
                    <td class="px-4 py-3 text-sm">
                      <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                        <img class="object-cover w-full h-full rounded-full" src="../img/Xe/ghe/<?php echo $row["KH_Hinh"]; ?>" loading="lazy" />
                    ...
                    <td class="px-4 py-3">
                      ...
                      <div class="flex items-center space-x-4 text-sm">
                        ...
                        <form action="" method="post">
                                                <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Sua" href="xemKH.php?this_id=<?php echo $row['KH_Ma'] ?> ">
                                                    <i class="fa fa-eye"></i>

                                                </a>
                                                <input type="hidden" name="KH_Ma" value="<?php echo $row['KH_Ma']; ?>" />
                                                </form>
                        <!-- SỬA -->
                        <form action="" method="post">
                          <a class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Sua" href="capnhatKH.php?this_id=<?php echo $row['KH_Ma'] ?> ">
                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <form action="" method="post">
                          <a style="color: red;" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Xoa" class="btn btn-info btn-sm" href="xoaKH.php?this_id=<?php echo $row['KH_Ma'] ?> ">
