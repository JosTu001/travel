...
// echo $this_id;
$sql = "SELECT * FROM chuyen a, nhanvien b, taixe c, xe d, tuyenchay f , benxe g where a.BXKH_Ma=g.BX_Ma and a.NV_Ma=b.NV_Ma and a.TX_Ma=c.TX_Ma and a.X_Ma=d.X_Ma and a.TC_Ma=f.TC_Ma and C_Ma = " . $this_id;
...
if (isset($_POST['chuyen'])) {
    ...
    if(((  strtotime($tgxp) < strtotime($ro['C_ThoiGianXP']) &&($tgxp > $ngayht)) &&   
    ...
    $sql = "UPDATE chuyen set NV_Ma='$nhanvien', TC_Ma='$tuyen', BXD_Ma='$benden', BXKH_Ma='$bendi', X_Ma='$xe',C_Hinh='$img', TX_Ma='$taixe', C_DonGia='$gia', C_ThoiGianXP='$tgxp', C_ThoiGianDen='$tgd' where C_Ma=" . $this_id;
    mysqli_query($conn, $sql);
    move_uploaded_file($img_tmp_name, '../img/Xe/ghe/' . $img);
    echo "<script>alert('Cập nhật chuyến thành công!');</script>"; 
    header('Refresh: 0;url=QLChuyen.php');
    // header('location:QLChuyen.php');
    ob_end_flush();
  }
  else{
                    ...
                    <div class="container px-6 mx-auto grid">
                        ...
                        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                            <label class="block text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Tên tuyến</span>
                                <select name="TC_Ma" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                            ...
                            <label class="block mt-4 text-sm">
                                ...
                                <input type="datetime-local" name="C_ThoiGianXP" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" VALUE="<?php echo $row["C_ThoiGianXP"];
                                                                                                                                                                                                                                                                                                    ?>" />
                            </label>
                            ...
                            <label class="block mt-4 text-sm">
                                ...
                                <input type="datetime-local" name="C_ThoiGianDen" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" VALUE="<?php echo $row["C_ThoiGianDen"];
                                                                                                                                                                                                                                                                                                        ?>" />
                            </label>

                            <label class="block text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Đơn giá</span>
                                <input  type="number" name="C_DonGia" min="50000" title="Giá phải là số và phải có giá trị lớn hơn 50.000"  class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" VALUE="<?php echo $row["C_DonGia"]; ?>" />
                            </label>

                            <label class="block mt-4 text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Xe</span>
                            ...
                            <label class="block mt-4 text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Bến xe khởi hành</span>
                                <select name="BXKH_Ma" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <option value="<?php echo $row['BXKH_Ma']; ?>"><?php echo $row["BX_Ten"]; ?> </option>
                                    <!-- <option value="" >Chọn xe</option>  -->
                                    <?php
                                    $sql = "SELECT * FROM benxe a,  quanhuyen b, tinhthanh c where a.H_Ma = b.H_Ma and b.T_Ma = c.T_Ma and T_Ten like '%".substr($row['TC_Ten'], 0, strpos($row['TC_Ten'], "-") - 1 )."%'  ";
                                    $result = mysqli_query($conn, $sql);
                                    while ($row = mysqli_fetch_array($result)) {
                                    ?>
                                        <option value="<?php echo $row['BX_Ma']; ?>"><?php echo $row['BX_Ten']; ?></option>
                                    <?php } ?>
                            ...
                            $sql = "SELECT * FROM chuyen a, nhanvien b, taixe c, xe d, tuyenchay f , benxe g where a.BXD_Ma=g.BX_Ma and a.NV_Ma=b.NV_Ma and a.TX_Ma=c.TX_Ma and a.X_Ma=d.X_Ma and a.TC_Ma=f.TC_Ma and C_Ma = " . $this_id;
                            $query = mysqli_query($conn, $sql);
                            $row = mysqli_fetch_assoc($query);

                            ?>

                                <select name="BXD_Ma" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <option value="<?php echo $row['BXD_Ma']; ?>"><?php echo $row["BX_Ten"]; ?> </option>
                                    <!-- <option value="" >Chọn xe</option>  -->
                                    <?php
                                    $sql = "SELECT * FROM benxe a,  quanhuyen b, tinhthanh c where a.H_Ma = b.H_Ma and b.T_Ma = c.T_Ma and T_Ten like '%".substr($row['TC_Ten'], strpos($row['TC_Ten'], "-") + 2)."%'";
                                    $re = mysqli_query($conn, $sql);
                                    while ($row = mysqli_fetch_array($re)) {
                                    ?>
                                        <option value="<?php echo $row['BX_Ma']; ?>"><?php echo $row['BX_Ten']; ?></option>
                            ...
                            <div class="text-center"><br>
                                <button   name="chuyen" class="  px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                                    Cập nhật
                                </button>
                            </div>
                        </div>
