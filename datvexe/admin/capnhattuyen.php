...
if (isset($_POST['tuyen'])) {
  ...
  $sql = "UPDATE tuyenchay set TC_Ten='$name',TC_Hinh='$img' where TC_Ma=" . $this_id;
  mysqli_query($conn, $sql);
  move_uploaded_file($img_tmp_name, '../img/Xe/ghe/' . $img);

  header('location:QLTuyen.php');
  ob_end_flush();
}
          ...
          <ul class="flex items-center flex-shrink-0 space-x-6">
            ...
            <li class="relative">
              ...
              <a class="btn  " data-toggle="dropdown" href="">
                <?php
                if (isset($_SESSION['mySesmySessionAdminsion'])) {
          ...
          <div class="container px-6 mx-auto grid">
            ...
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              <label class="block text-sm">
                ...
                <select  name="diemdi" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                   
                <option style="text-align: center;" value="<?php echo substr($row['TC_Ten'], 0, strpos($row['TC_Ten'], "-") - 1); ?>"><?php echo substr($row['TC_Ten'], 0, strpos($row['TC_Ten'], "-") - 1); ?></option>
                                    <?php
                                    $di = "SELECT * FROM tinhthanh  ";
                                    $r = mysqli_query($conn, $di);
                                    while ($di = mysqli_fetch_array($r)) {
                                    ?>
                                         
                                        <option value="<?php echo $di['T_Ten']; ?>"><?php echo $di['T_Ten']; ?></option>
                                    <?php } ?>

                                </select>&emsp;&emsp; <i class="fa fa-arrow-right" aria-hidden="true"></i> &emsp;&emsp;
                                <select name="diemden" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <option  style="text-align: center;" value="<?php echo substr($row['TC_Ten'], strpos($row['TC_Ten'], "-") + 2);?>"><?php echo substr($row['TC_Ten'], strpos($row['TC_Ten'], "-") + 2);?></option>

                                    <?php
                                    $den = "SELECT * FROM tinhthanh  ";
              ...
              <!-- <label class="block mt-4 text-sm">
                ...
                <input type="date" name="TC_Thoigian" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" VALUE="<?php //echo $row["TC_ThoiGian"]; 
                                                                                                                                                                                                                                                                          ?>" />
              </label> -->
              ...
              <div class="text-center"><br>
                <button name="tuyen" class="  px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                  Cập nhật
                </button>
              </div>
            </div>
