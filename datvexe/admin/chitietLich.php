...
<body>
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen}">
      ...
      <main class="h-full pb-16 overflow-y-auto">
        ...
        $sql = "SELECT * FROM taixe a, xe b, tuyenchay c, chuyen d, ghe e, loaixe f where b.X_Ma=e.X_Ma and f.LX_Ma=b.LX_Ma and  a.TX_Ma = d.TX_Ma and b.X_Ma = d.X_Ma and c.TC_Ma = d.TC_Ma and C_ThoiGianDen > NOW() and d.C_Ma='$this_id' ORDER BY C_ThoiGianXP ";
    ...
    //   }
        ...
        <div class="flex-container" style="margin-bottom: 0rem;">
                <div  class="sidebarTieuDe"><h2 class="text-center"style="font-size:18px; color:darkblue; "  ><i class="fa fa-check"></i> <b >Thông tin chuyến</b></h2></div>
                <div class="main-contentTieuDe"><h2 class="text-center"style="font-size:18px; color:darkblue"  ><i class="fa fa-car"></i> <b >Thông tin xe</b></h2></div>
            </div>
        ...
        <div class="sidebar">
                        ...
                        <li ><b>Mã chuyến: </b><?php echo $row["C_Ma"]; ?></li>
                        <li ><b>Tên chuyến: </b><?php echo $row["TC_Ten"]; ?></li>
                        <li><b>Thời gian khởi hành: </b><?php  $date = date_create($row["C_ThoiGianXP"]);
                            echo date_format($date, "d-m-Y (H:s:i)") ; ?></li>

                        <li><b>Thời gian đến: </b><?php  $date = date_create($row["C_ThoiGianDen"]);
                            echo date_format($date, "d-m-Y (H:s:i)") ; ?></li>
                    
                        <li><b>Bến xe khởi hành: </b>
                        <?php
                                        $s = "SELECT * FROM chuyen a, benxe b where a.BXKH_Ma=b.BX_Ma and C_Ma = '".$row['C_Ma']."'";
                                        $q = mysqli_query($conn, $s);
                                        $r = mysqli_fetch_assoc($q);
                                        ?>
                                        <?php echo $r["BX_Ten"]; ?></li>
                        <li><b>Bến xe đến: </b>
                        <?php
                                        $den = "SELECT * FROM chuyen a, benxe b where a.BXD_Ma=b.BX_Ma and C_Ma = '".$row['C_Ma']."'";
                                        $quee = mysqli_query($conn, $den);
                                        $o = mysqli_fetch_assoc($quee);
                                        ?>
                                        <?php echo $o["BX_Ten"]; ?></li>
                        ...
                        <!-- <a style=" font-size:16px; width: 40%; margin-left:6rem" class=" block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" href="capnhatTX.php?this_id=<?php echo $row['TX_Ma'] ?> ">
                        SỬA
                        </a> -->
                    </div>
                    <div class="main-content"><ul class="list-unstyled booking-info">
                                    <li ><b>Mã xe: </b><?php echo $row["X_Ma"]; ?></li>
                                    <li ><b>Biển kiểm soát: </b><?php echo $row["X_BienSo"]; ?></li>
                                    <li><b>Màu sắc: </b><?php  echo $row["X_MauSac"]; ?></li>
                                    <li><b>Loại xe: </b><?php  echo $row["LX_Ten"]; ?></li>

                                    <li><b>Số lượng ghế: </b><?php  echo $row["LX_SLGhe"];?> (Ghế)</li>
                                    <li><b>Số lượng ghế đã đặt: </b>
                                    <?php   
                                        $s =  "SELECT count(*) as 'dadat' FROM taixe a, xe b, tuyenchay c, chuyen d, ghe e, loaixe f where b.X_Ma=e.X_Ma and f.LX_Ma=b.LX_Ma and  a.TX_Ma = d.TX_Ma and b.X_Ma = d.X_Ma and c.TC_Ma = d.TC_Ma and C_ThoiGianDen > NOW() and d.C_Ma='$this_id' and G_TrangThai='1' ORDER BY C_ThoiGianXP ";
                                        $r = mysqli_query($conn, $s);
                                        $o = mysqli_fetch_assoc($r);
                                        echo "<span style='border-radius:2rem;background-color: red;color: #fff;box-shadow: 0px 0px 10px tomato;padding:1px'>".$o["dadat"]." (Ghế)</span>";
                                    ?>
                                    </li>
                                    <li><b>Số lượng ghế còn trống: </b>
                                    <?php   
                                        $s =  "SELECT count(*) as 'controng' FROM taixe a, xe b, tuyenchay c, chuyen d, ghe e, loaixe f where b.X_Ma=e.X_Ma and f.LX_Ma=b.LX_Ma and  a.TX_Ma = d.TX_Ma and b.X_Ma = d.X_Ma and c.TC_Ma = d.TC_Ma and C_ThoiGianDen > NOW() and d.C_Ma='$this_id' and G_TrangThai='0' ORDER BY C_ThoiGianXP ";
                                        $r = mysqli_query($conn, $s);
                                        $o = mysqli_fetch_assoc($r);
                                        echo "<span style='border-radius:2rem;background-color:  #009688;color: #fff;box-shadow: 0px 0px 10px yellow;padding:1px'>".$o["controng"]." (Ghế)</span>";
                                    ?> 
                                    </li>
                                    ...
                                    <!-- You should use a button here, as the anchor is only used for the example  -->
                                    <!-- <a  style=" font-size:16px; width: 40%; margin-left:6rem;text-align: center;" class=" block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" href="capnhatTX.php?this_id=<?php echo $row['TX_Ma'] ?> ">
                                    SỬA
                                    </a> -->
                                </div>
                    </div>
        ...
        <div class="container px-6 mx-auto grid">
         ...
         <h2 style="font-size:18px; color:darkblue" class="text-center" ><i class="fa fa-braille" ></i> <b >Tình trạng ghế</b></h2>
        
          <div class="flex items-center  p-6 bg-gray-50 dark:bg-gray-900">
            <div style="margin-top: 1rem;border: 1px solid #ddd; padding: 12px; background-color:white;border-radius:10px;" class="flex-1 h-full mx-auto    ">
            <style>
            ...
            <?php
                            $i=0;
                            $sl =  "SELECT * FROM taixe a, xe b, tuyenchay c, chuyen d, ghe e, loaixe f where b.X_Ma=e.X_Ma and f.LX_Ma=b.LX_Ma and  a.TX_Ma = d.TX_Ma and b.X_Ma = d.X_Ma and c.TC_Ma = d.TC_Ma and C_ThoiGianDen > NOW() and d.C_Ma='$this_id' ORDER BY C_ThoiGianXP ";
                           ...
                           <th style="width: 10%;text-align:center;">Mã số ghế</th>
                           <th style="width: 20%;text-align:center;padding:20px;">Trạng thái</th>
                          ...
                          <?php
                             while ($ro = mysqli_fetch_array($re)) { 
                                if ($ro['G_TrangThai'] == 1) {
                                    $ro['G_TrangThai'] = "Đã đặt ";
                                }else {
                                    $ro['G_TrangThai'] = "Trống";
                                }
                                $i++;
                                if($ro['G_TrangThai'] == "Đã đặt "){
                        ?> 
                         <tr style="text-align:center; background-color: pink;">
                         <td><?php echo $i ?></td>
                       
                        <td ><?php echo $ro['G_MaSoGhe'] ?> </td>
                        <!-- <td> <?php echo $ro['TC_Ten'] ?></td>    -->
                        <td> <?php echo $ro["G_TrangThai"]?></td>
                       
                       
                        <!-- <td ><a href="chitiet.php?this_id=<?php echo $ro['C_Ma'] ?>" style="color: blue;text-decoration: none; cursor:pointer"><b>Chi tiết <i class="fa fa-eye"></i></b></a> </td> -->
