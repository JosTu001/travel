...
if (isset($_POST['sua'])) {
        ...
        $sql = "UPDATE benxe set BX_Ten='$ten', H_Ma='$huyen' where BX_Ma = '$ben'";
        mysqli_query($conn, $sql);
    
        header("location:QLBX.php");
    }
            ...
            <main class="h-full pb-16 overflow-y-auto">
                    ...
                    <div class="container px-6 mx-auto grid">
                        ...
                        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                            ...
                            <?php
                            $sql = "SELECT * FROM benxe a, quanhuyen b, tinhthanh c where a.H_Ma = b.H_Ma and b.T_Ma = c.T_Ma and  BX_Ma = '$id'";
                            $query = mysqli_query($conn, $sql);
                            $row = mysqli_fetch_assoc($query);
                           
                          
                            ?>
                         <form action="suaBX.php" method="post" enctype="multipart/form-data">
                            <label class="block mt-4 text-sm">
                                <span class="text-gray-700 dark:text-gray-400">
                                   Tên bến xe
                                </span>
                                <input type="text" name="tenben" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" VALUE="<?php echo $row["BX_Ten"];
                                                                                                                                                                                                                                                                                    ?>" />
                            </label>

                            <label class="block mt-4 text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Tên Tỉnh/Thành phố</span>
                                <select   name="tinh" class="tinh block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <option  value="<?php echo $row['T_Ma']; ?>"><?php echo $row['T_Ten']; ?></option>
                                    <?php
                            ...
                            <label class="block mt-4 text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Tên Quận/Huyện</span>
                                <select  name="huyen" class="huyen block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <option value="<?php echo $row['H_Ma']; ?>"  ><?php echo $row['H_Ten']; ?></option>
                                </select> 
                            </label>


                            <div class="text-center"><br>
                            <input type="hidden" name="ben" value="<?php echo $row['BX_Ma']; ?>" />
                                <button name="sua" type="submit" class="  px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                                    Cập nhật
                                </button>
                            </div>
                            </form>
