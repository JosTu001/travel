    ...
    <div class="flex flex-col flex-1 w-full">
      <header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
        <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
          ...
          <form method="post" class="flex justify-center flex-1 lg:mr-32">
            ...
            </div> <button name="timkiem" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">Tìm kiếm</button>
      ...
      <main class="h-full pb-16 overflow-y-auto">
          ...
          <div class="flex items-center  p-6 bg-gray-50 dark:bg-gray-900">
            <div class="flex-1 h-full mx-auto  bg-white  shadow-xl dark:bg-gray-800">
              ...
              <div class="flex flex-col overflow-y-auto md:flex-row">
                <div style="margin-top: 3rem;margin-bottom: 1rem;padding-left: 10rem" class="h-32 md:h-auto md:w-1/2">
                  <img width="70%" class="" src="../img/Xe/ghe/<?php echo $row["KH_Hinh"]; ?>" loading="lazy" />
                </div>
                <div class=" items-center justify-center p-6 sm:p-12 md:w-1/2">


                  <h3 ><b>Họ tên: </b><?php echo $row["KH_Ten"]; ?></h3>
                  <ul class="list-unstyled booking-info">
                    <li><b>Ngày sinh: </b><?php  $date = date_create($row["KH_NgaySinh"]);
                        echo date_format($date, "d-m-Y ") ; ?></li>
                
                    <li><b>Giới tính: </b><?php echo $row["KH_GioiTinh"]; ?></li>
                    <li><b>Email: </b><?php echo $row["KH_Email"]; ?></li>
                    <li><b>Số điện thoại: </b><?php echo $row["KH_Sdt"]; ?></li>
                    <li><b>Địa chỉ: </b><?php echo $row["KH_DiaChi"]; ?></li>
              
                
                  </ul>

                  <!-- You should use a button here, as the anchor is only used for the example  -->
                  <a style=" font-size:16px; width: 40%;" class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" href="capnhatKH.php?this_id=<?php echo $row['KH_Ma'] ?> ">
                    SỬA
                  </a>
                </div>
        ...
        <div class="container px-6 mx-auto grid">
         ...
         <h2 style="font-size:18px; color:darkblue" class="text-center" ><i class="fa fa-ticket" ></i> <b >Vé đã đặt</b></h2>
          ...
          <div class="flex items-center  p-6 bg-gray-50 dark:bg-gray-900">
            ...
            <?php
                            $i=0;
                            $sl = "SELECT * FROM  phieudatve a, chuyen b, tuyenchay c WHERE a.C_Ma=b.C_Ma and b.TC_Ma=c.TC_Ma and KH_Ma='$this_id' order by PD_NgayDat DESC";
                            $re = mysqli_query($conn, $sl);
                            
                                if(mysqli_num_rows($re) == false){ ?>
                                <h4 class="text-center"  style="color:blue">
                                    <span style="color: red;margin:2rem;">(Hiện tại khách hàng chưa đặt vé nào)</span> 
                                </h4>
                                <?php
                           ...
                           <th style="width: 9%;text-align:center;">STT</th>
                           <th style="width: 10%;text-align:center;">Mã vé</th>
                           <th style="width: 15%;text-align:center;padding:20px;">Thời gian đặt</th>
                            <th style="width: 25%;text-align:center;">Chuyến</th>
                            <th style="width: 13%;text-align:center;">Số ghế đặt</th>
                          ...
                          <?php
                             while ($ro = mysqli_fetch_array($re)) { 
                                ...
                                if ($ro['PD_TrangThai'] == 1) {
                                    $ro['PD_TrangThai'] = "Đặt thành công";
                                } else if ($ro['PD_TrangThai'] == 2) {
                                    $ro['PD_TrangThai'] = "Đã hoàn thành";
                                }
                                else if ($ro['PD_TrangThai'] == 3) {
                                    $ro['PD_TrangThai'] = "Đã bị xoá";
                                }else {
                                    $ro['PD_TrangThai'] = "Đã huỷ";
                                }
                            $i++;
                        ?> 
                         <tr style="text-align:center;">
                       <td><?php echo $i ?></td>
                       <td><?php echo  $ro['PD_Ma'] ?></td>
                        <td ><?php $date=date_create($ro["PD_NgayDat"]); echo date_format( $date, " d-m-Y (H:i:s)");   ?> </td>
                        <td> <?php echo "".$ro['TC_Ten']." (".$ro['C_Ma'].")" ?></td>   
                        
                        <?php
                                     $s = "select count(G_Ma) as 'tong' from chitietphieudat where PD_Ma ='" . $ro['PD_Ma'] . "' ";
                                     $q = mysqli_query($conn, $s);
                                     $r = mysqli_fetch_assoc($q);
                                     ?>
                        <td> <?php echo $r['tong'] ?></td>
                        <td> <?php echo $ro["PD_TrangThai"]?></td>
                        <td ><a href="xemPD.php?this_id=<?php echo $ro['PD_Ma'] ?>" style="color: blue;text-decoration: none; cursor:pointer"><b>Chi tiết </b></a> </td>
