...
<?php
   ...
   // echo $this_id;
    $sql="SELECT * FROM  khachhang b,phieudatve a,chitietphieudat k, xe c, ghe d, tuyenchay f, loaixe g, taixe i, chuyen e , thanhtoan s WHERE k.PD_Ma=a.PD_Ma and s.PD_Ma=a.PD_Ma and e.TC_Ma=f.TC_Ma and a.KH_Ma=b.KH_Ma AND a.C_Ma=e.C_Ma and k.G_Ma=d.G_Ma and d.X_Ma=c.X_Ma and c.LX_Ma=g.LX_Ma  and K.PD_Ma = ".$this_id;
    $query =mysqli_query($conn,$sql);
    $row = mysqli_fetch_assoc($query);
   

   if(isset($_POST['suave'])){
        //$id = "";
        $name=$_POST['name'];
       
        $email = $_POST['email'];
        $sdt = $_POST['sdt'];
        $diemdon = $_POST['PD_DiemDon'];
      
        $sql="UPDATE chitietphieudat set CT_TenKH='$name',CT_EmailKH='$email',CT_SdtKH='$sdt' where PD_Ma=".$this_id;
        mysqli_query($conn,$sql);

        $pd="UPDATE phieudatve set PD_DiemDon='$diemdon' where PD_Ma=".$this_id;
        mysqli_query($conn,$pd);
...
<body id="main-homepage">
    ...
    <nav class="navbar navbar-default main-navbar navbar-custom navbar-white" id="mynavbar-1">
        <div class="container">
            <div class="navbar-header">
                ...
                <a href="index.php" class="navbar-brand"><span><i class="fa fa-car"></i>VAN</span> TU</a>
    ...
    <div class="sidenav-content">
        <div id="mySidenav" class="sidenav">
            <h2 id="web-name"><span><i class="fa fa-car"></i></span>VAN TU</h2>
	...
	">
            <div class="container">
                <div class="row">
                    <div class="col-sm-12">
                    	<h1 class="page-title">CHỈNH SỬA THÔNG TIN VÉ</h1>
                        ...
                        <div style="padding-left:30%;padding-right:30% ;margin: 1rem auto; max-width: 60rem; padding: 1rem;">
                        	...
                        	<div class="flex-content">
                                <div class="custom-form custom-form-fields">
                                    <h3 class="text-center"><B>CHỈNH SỬA THÔNG TIN VÉ</B></h3>
                                   
                                    <form method="post" enctype="multipart/form-data">
                                            
                                        <div class="form-group">
                                             <input name="name" type="text" class="form-control" placeholder="Họ tên" VALUE="<?php  echo $row["CT_TenKH"]; ?>"  required/>
                                             <span><i class="fa fa-user"></i></span>
                                        </div>


                                        <div class="form-group">
                                             <input name="sdt" type="tel" class="form-control"  placeholder="Số điện thoại"  pattern="(\+84|0)\d{9,10}"  title="Số điện thoại phải bắt đầu từ số 0 và có độ dài từ 10 đến 11 số"   placeholder="Số điện thoại" VALUE="<?php  echo $row["CT_SdtKH"]; ?>" required/>
                                             <span><i class="fa fa-phone"></i></span>
                                        </div>
        
                                        <div class="form-group">
                                             <input name="email" type="email" class="form-control" placeholder="Email" VALUE="<?php  echo $row["CT_EmailKH"]; ?>"  required/>
                                             <span><i class="fa fa-envelope"></i></span>
                                        </div>                                      
        
                                        <div class="form-group">
                                             <input  name="PD_DiemDon" type="text" class="form-control" placeholder="Điểm lên xe"  VALUE="<?php  echo $row["PD_DiemDon"]; ?>"  required/>
                                             <span><i class="fa fa-map-marker"></i></span>
                                        </div>                                                                                                                                                                                                                                                                                             
                                        <button name="suave"  class="btn btn-orange btn-block">Cập nhật</button>
                                    </form>
